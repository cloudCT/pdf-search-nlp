# Embedding Script: all-mpnet-base-v2
# Chunks and embeds scientific papers using MPNet.
...
def process_corpus_mpnet(chunk_size=384, overlap=64, batch_size=32, device="mps"):
    """
    Processes all .txt files in data/processed/, chunks them, embeds them, and saves embeddings + metadata to data/embeddings/mpnet/.
    Args:
    ...
    output_dir = os.path.join(DATA_PATH, "embeddings", "mpnet")
...
def main():
    parser = argparse.ArgumentParser(description="Chunk and embed scientific papers using all-mpnet-base-v2.")
    ...
    process_corpus_mpnet(args.chunk_size, args.overlap, batch_size=args.batch_size, device=args.device)
