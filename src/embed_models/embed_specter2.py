# Embedding Script: allenai/specter2_base
# Chunks and embeds scientific papers using SPECTER2.
# Default chunk size: 384 (override with --chunk_size)


This script chunks and generates embeddings for scientific papers using the allenai/specter2_base model.
...
Usage:
    python embed_specter2.py --chunk_size <int> --overlap <int>

Example:
    python embed_specter2.py --chunk_size 384 --overlap 64
...
def process_corpus_sp2(chunk_size, overlap, batch_size=32, device="mps"):
    """
    Processes all .txt files in data/processed/, chunks them, embeds them, and saves embeddings + metadata to data/embeddings/specter2/.
    Args:
    ...
    output_dir = os.path.join(DATA_PATH, "embeddings", "specter2")
    print("Loading model: allenai/specter2_base")
...
def main():
    parser = argparse.ArgumentParser(description="Chunk and embed scientific papers using allenai/specter2_base.")
    ...
    process_corpus_sp2(args.chunk_size, args.overlap, batch_size=args.batch_size, device=args.device)
