# Embedding Script: jinaai/jina-embeddings-v2-base-en
# Chunks and embeds scientific papers using Jina V2.
# Default chunk size: 384 (override with --chunk_size)


This script chunks and generates embeddings for scientific papers using the jinaai/jina-embeddings-v3-base-en model.
...
Usage:
    python embed_jina_v2.py --chunk_size <int> --overlap <int>

Example:
    python embed_jina_v2.py --chunk_size 384 --overlap 64
...
def process_corpus_jina(chunk_size, overlap, batch_size=32, device="mps"):
    """
    Processes all .txt files in data/processed/, chunks them, embeds them, and saves embeddings + metadata to data/embeddings/jina_v3/.
    Args:
    ...
    output_dir = os.path.join(DATA_PATH, "embeddings", "jina_v2")

    print("Loading model: jinaai/jina-embeddings-v2-base-en")
    ...
    print("Done. Embeddings and metadata saved to {}".format(output_dir))



## Function call for testing

process_corpus_jina(384, 64)
...
def main():
    parser = argparse.ArgumentParser(description="Chunk and embed scientific papers using jinaai/jina-embeddings-v3-base-en.")
    ...
    process_corpus_jina(args.chunk_size, args.overlap, batch_size=args.batch_size, device=args.device)
